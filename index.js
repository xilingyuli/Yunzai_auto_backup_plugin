...
function avatarHash (avatarData) {
  ...
  // 圣遗物只比身份标识字段，排除会变化的attrIds
  let artisSig = {}
  for (let [slot, arti] of Object.entries(artis)) {
    if (arti && typeof arti === 'object') {
      artisSig[slot] = { level: arti.level, name: arti.name, star: arti.star, mainId: arti.mainId }
    }
  }

  let weaponSig = weapon && typeof weapon === 'object'
    ? { name: weapon.name, level: weapon.level, promote: weapon.promote, affix: weapon.affix }
    : {}

  let sig = JSON.stringify({ artis: artisSig, weapon: weaponSig, talent, cons, level })
